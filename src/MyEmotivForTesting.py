...
class MyEmotiv(object):
    ...
    def loadData(self, file_name):
        with open(file_name, "r") as csv_file:
            return self.getRows(csv.DictReader(csv_file), self.toInt)
    ...
    def start(self):
        # self.startDataHandling()
        data = self.loadData("C:\\Users\\Anti\\Desktop\\PycharmProjects\\VEP-BCI\\src\\cca_data_generator\\eeg.csv")
        # print data
        i = 0
        while True:
            try:
                ...
                self.connection.sendMessage(data[i])
                i += 1
