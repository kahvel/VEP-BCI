...
class TestItrCalculatorProb(TestCase):
    ...
    def test_entropyPderivative(self):
        ...
        expected = [-sum(map(lambda (df, f): df * (np.log(f) + 1) / np.log(2), zip(pi_d, pi))) for pi_d in
                    pi_derivative]
        self.assertEqual(actual, expected)
    ...
    def test_entropyOfPgivenCderivative(self):
        ...
        expected = [-sum(map(lambda (df, f): df * (np.log(f) + 1) / np.log(2), zip(pi_d, pi_given_cj))) for pi_d in
                    pi_given_cj_derivative]
        np.testing.assert_almost_equal(actual, expected, 16)
    ...
    def test_entropyPgivenCderivative(self):
        ...
        entropy_of_p_given_c_derivative = self.calculator.entropyOfPgivenCderivative(prob_pi_given_cj,
                                                                                     prob_pi_given_cj_derivative)
        actual = self.calculator.entropyPgivenCderivative(prob_ck_derivative,
                                                          self.calculator.entropyOfPgivenC(prob_pi_given_cj), prob_ck,
        ...
        entropy_of_p_given_c = -sum(map(lambda x: x * np.log2(x), pi_given_cj))
        entropy_of_p_given_c_derivative = [
            -sum(map(lambda (df, f): df * (np.log(f) + 1) / np.log(2), zip(pi_d, pi_given_cj))) for pi_d in
            pi_given_cj_derivative]
        prob_ck = self.calculateProbCk(R)
    ...
    def test_mutualInformationDerivative(self):
        ...
        entropy_of_p_given_c_derivative = self.calculator.entropyOfPgivenCderivative(prob_pi_given_cj,
                                                                                     prob_pi_given_cj_derivative)
        actual_entropy_p_given_c_derivative = self.calculator.entropyPgivenCderivative(prob_ck_derivative,
                                                                                       self.calculator.entropyOfPgivenC(
                                                                                           prob_pi_given_cj), prob_ck,
                                                                                       entropy_of_p_given_c_derivative)
        ...
        prob_ck = self.calculateProbCk(R)
        prob_ck_derivative = self.calculateProbCkDerivative(R, R_derivative)
        expected_entropy_p_given_c_derivative = [
            sum(dc * entropy + de * ck for ck, entropy, de, dc in zip(prob_ck, entropy_of_p_given_c, d_entropy, d_ck))
            for d_entropy, d_ck in zip(entropy_of_p_given_c_derivative, prob_ck_derivative)]
        ...
        pi = self.calculatePi(R)
        pi_derivative = self.calculatePiDerivative(R, R_derivative)
        expectedentropy_p_derivative = [-sum(map(lambda (df, f): df * (np.log(f) + 1) / np.log(2), zip(pi_d, pi))) for
                                        pi_d in pi_derivative]

        actual = self.calculator.mutualInformationDerivative(actual_entropy_p_derivative,
                                                             actual_entropy_p_given_c_derivative)
    ...
    def test_itrDerivative(self):
        ...
        entropy_p_given_c_derivative = self.calculator.entropyPgivenCderivative(prob_ck_derivative,
                                                                                entropy_of_p_given_c, prob_ck,
                                                                                entropy_of_p_given_c_derivative)
        mutual_information = self.calculator.mutualInformation(entropy_p, entropy_p_given_c)
        mutual_information_derivative = self.calculator.mutualInformationDerivative(entropy_p_derivative,
                                                                                    entropy_p_given_c_derivative)
        mdt = self.calculator.mdt(R)
        mdt_derivative = self.calculator.mdtDerivative(R, R_derivative)
        actual = self.calculator.itrMiDerivative(mutual_information, mutual_information_derivative, mdt, mdt_derivative)

        R_derivative = self.calculateRderivative()
        expected_mdt_derivative = [-self.step / R ** 2 * dr for dr in R_derivative]
        expected_mdt = self.window_length + (
                                            self.feature_maf_length + self.proba_maf_length + self.look_back_length + 1.0 / R - 4) * self.step
        ...
        expected_mi_derivative = [a - b for a, b in
                                  zip(expectedentropy_p_derivative, expected_entropy_p_given_c_derivative)]

        expected = [(ditr * mdt - dmdt * expected_mi) / expected_mdt ** 2 * 60 for ditr, dmdt in
                    zip(expected_mi_derivative, expected_mdt_derivative)]
